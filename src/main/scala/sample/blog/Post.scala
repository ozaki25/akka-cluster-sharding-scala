...
class Post(authorListing: ActorRef) extends PersistentActor with ActorLogging {
  ...
  // 状態を復元するときに過去のメッセージが飛んでくる
  override def receiveRecover: Receive = {
    case evt: PostAdded =>
      context.become(created)
      state = state.updated(evt)
    // a @ addPost(postId) てするとaはまるごと、postIdはidが入る
    // objectの時はa:PostPublishedて書けないa:PostPublished.typeならok
    // type使わないなら以下の書き方
  ...
  // becomeでころころ変えてるけどunhandleは一か所に書いてる
